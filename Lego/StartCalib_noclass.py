...
def FindBlobLocation(hsv_color_image, color_name):
    ...
    for keypoint in keypoints:
        ...
        y = keypoint.pt[1]
        blob_location_list.append([y,x,1])

    blob_location_dict[color_name] = blob_location_list
    return blob_location_list

detector=cv.SimpleBlobDetector_create()

blob_location_dict = {}
for color_name in detected_color_dict:
    blob_locations_list = FindBlobLocation(hsv_color_image = detected_color_dict.get(color_name), color_name = color_name)
    blob_location_dict[color_name] = blob_locations_list

print(blob_location_dict)

#-------------------------------------------------------
#Extract the patches
def ExtractPatches(blob_location,colored_image):
    patch_center = np.array([blob_location[0], blob_location[1]]) # the location of the patch
...
for color_name in blob_location_dict:
    for idx, blob in enumerate(blob_location_dict.get(color_name)):
        extracted_patch = ExtractPatches(blob,detected_color_dict.get(color_name))
        resized_patch = ResizePatch(extracted_patch)
        cv.imshow(f'{color_name} Blob{idx}',resized_patch)
